...
export const teacherAssignmentQueries = {
  ...
  async getCorrectExamRoomId(roomNumber: string, examDate: string, session: ExamSession): Promise<number | null> {
  ...
  async getAvailableTeachersWithSession(
    ...
    return eligibleTeachers.map(t => ({
        ...
        availability: {
            ...
            conflict_reason: busyIds.includes(t.teacher_id) ? "Already Assigned" : null
        },
        workload_level: getWorkloadLevel(t.total_periods_assigned)
    })).sort((a, b) => (a.availability.is_available === b.availability.is_available ? 0 : a.availability.is_available ? -1 : 1));
  },
